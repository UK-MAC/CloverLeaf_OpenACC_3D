...
! details.
!
! You should have received a PCOPY of the GNU General Public License along with 
...
!$ACC LOOP INDEPENDENT 
  ...
  ENDDO
!$ACC END KERNELS
!$ACC UPDATE HOST (back_snd_buffer)
!$ACC END DATA
...
!$ACC UPDATE DEVICE (back_rcv_buffer)
!$ACC KERNELS
!$ACC LOOP INDEPENDENT
  DO l=1,depth
!$ACC LOOP INDEPENDENT
...
!$ACC LOOP INDEPENDENT 
  ...
  ENDDO
!$ACC END KERNELS
!$ACC UPDATE HOST (front_snd_buffer)
!$ACC END DATA
...
!$ACC UPDATE DEVICE (front_rcv_buffer)
!$ACC KERNELS
!$ACC LOOP INDEPENDENT
  DO l=1,depth
!$ACC LOOP INDEPENDENT
